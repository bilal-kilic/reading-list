...
export class ArticleColumn extends React.Component<ArticleColumnProps, ArticleColumnState> {
    ...
    componentDidMount() {
        async function fetchData(page: number = 0, pageSize: number = 20, isRead: boolean = false, articleType: ArticleType, subreddit: string) {
            switch (articleType) {
                case "ARTICLE":
                    return get<PageResponse<ArticleDto>>(`${process.env.REACT_APP_BACKEND_URL}/articles?articleType=ARTICLE&isRead=${isRead}&page=${page}&pageSize=${pageSize}`)
                case "REDDIT":
                    return get<PageResponse<RedditArticleDto>>(`${process.env.REACT_APP_BACKEND_URL}/articles/reddit?articleType=REDDIT&isRead=${isRead}&page=${page}&pageSize=${pageSize}&subreddit=${subreddit}`)
            }
        }
